...
Route::group(['prefix' => 'webhook'], function () {
    Route::post('/line-alert', [App\Http\Controllers\Api\ErrorController::class, 'LineWebhook']);
    Route::post('/send-alert', [App\Http\Controllers\Api\ErrorController::class, 'SendAlert']);
});
...
Route::group(['middleware' => 'api'], function ($router) {
    Route::group(['middleware' => 'api.key'], function ($router) {
        ...
        // 官網IP市集投票系統
        Route::prefix('ip-shops-activity')->group(function () {
            // 獲取所有 IP 商店列表（按投票數排序）
            Route::get('/', [App\Http\Controllers\Api\WebIpActivityShopController::class, 'index']);

            // 投票功能
            Route::post('/{id}/vote', [App\Http\Controllers\Api\WebIpActivityShopController::class, 'vote']);

            // 獲取投票排行榜
            Route::get('/leaderboard', [App\Http\Controllers\Api\WebIpActivityShopController::class, 'leaderboard']);
        });
        ...
        // 重置玩家Surgame資料（僅測試環境）
        Route::post('/reset_user_surgame_data', [App\Http\Controllers\Api\AdminsController::class, 'resetUserSurgameData']);

        // 戰鬥系統
        Route::group(['prefix' => 'battle'], function () {
            Route::post('/revive', [App\Http\Controllers\Api\BattleController::class, 'revive']);
        });
        ...
        Route::group(['prefix' => 'character'], function () {
            ...
            Route::group(['prefix' => 'user_character'], function () {
                ...
                // 重置人物等級
                Route::post('/reset_character_lv_up', [App\Http\Controllers\Api\CharacterController::class, 'resetMainCharacterLevel']);
            });
            ...
            Route::group(['prefix' => 'star_rank'], function () {
                ...
                // 星級重置
                Route::post('/level_reset', [App\Http\Controllers\Api\CharacterController::class, 'startLevelReset']);
            });
        });
        ...
        // 現金商品
        Route::group(['prefix' => 'cash_mall'], function () {
            // 取得現金商品
            Route::get('/lists', [App\Http\Controllers\Api\ItemPriceController::class, 'getCashMallItems']);


            Route::group(['prefix' => 'month_card'], function () {
                // 領取每日獎勵
                Route::post('/claim_daily', [App\Http\Controllers\Api\MonthCardController::class, 'claimDailyReward']);

                // 重置月卡（測試用）
                Route::post('/reset_month_card', [App\Http\Controllers\Api\MonthCardController::class, 'resetMonthlyCardStatus']);
                // 購買月卡（測試用）
                Route::post('/purchase_month_card', [App\Http\Controllers\Api\MonthCardController::class, 'purchaseMonthCard']);
            });
        });
        ...
        Route::group(['prefix' => 'month_card'], function () {
            // 取得現金商品
                Route::get('/lists', [App\Http\Controllers\Api\MonthCardController::class, 'index']);
                // 領取每日獎勵
                Route::post('/claim_daily', [App\Http\Controllers\Api\MonthCardController::class, 'claimDailyReward']);
                // 重置月卡（測試用）
                Route::post('/reset_month_card', [App\Http\Controllers\Api\MonthCardController::class, 'resetMonthlyCardStatus']);
        ...
        Route::group(['prefix' => 'grade'], function () {
            ...
            // 領取獎勵
            Route::post('/claim_reward', [App\Http\Controllers\Api\UserGradeController::class, 'claimGradeReward']);
        });
        ...
        // surgame資源關卡
        Route::group(['prefix' => 'surgame_resource_stage'], function () {
            // 清除關卡（首通）
            Route::post('/{type}/mark_cleared', [App\Http\Controllers\Api\UserSurgameResourceController::class, 'markCleared']);
            // 掃蕩關卡
            Route::post('/{type}/sweep', [App\Http\Controllers\Api\UserSurgameResourceController::class, 'sweepStage']);
            // 取得玩家目前進度
            Route::get('/progress', [App\Http\Controllers\Api\UserSurgameResourceController::class, 'getUserStageProgress']);
        });
    });
