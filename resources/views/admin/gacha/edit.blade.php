                    ...
                    <div class="card-body">
                        <form ref="myForm" autocomplete="off">
                            <div class="form-row">
                                ...
                                <div class="form-group col-md-6">
                                    <label>{{ __('本地化名稱') }}</label>
                                    <input type="text" class="form-control" v-model="editData.localization_name"
                                        placeholder="{{ __('本地化名稱') }}" maxlength="255">
                                </div>
                            </div>
                            <div class="form-group">
                                <label>{{ __('類型') }}<span class="red">*</span></label>
                                <select v-model="editData.type" class="form-control" required>
                                    <option :value="1">{{ __('1 - 英雄卡池') }}</option>
                                    <option :value="2">{{ __('2 - 寶物卡池') }}</option>
                                    <option :value="3">{{ __('3 - 造型卡池') }}</option>
                                    <option :value="4">{{ __('4 - 活動卡池') }}</option>
                                </select>
                            </div>
                            ...
                            <div class="form-row">
                                <div class="form-group col-md-6">
                                    <label>{{ __('免費抽次數') }}</label>
                                    <input type="number" class="form-control" v-model="editData.free_draw_times"
                                        placeholder="{{ __('免費抽次數') }}" min="0">
                                </div>
                            </div>
                            <div class="form-row">
                                <div class="form-group col-md-6">
                            ...
                            <div class="form-row mt-4">
                                <button type="button" class="btn btn-primary col-12" @click="checkSave">
                                    {{ __('確認儲存') }}
                                </button>
                            </div>
                        </form>
    ...
    <script type="text/javascript">
        var vc = new Vue({
            ...
            data: {
                ...
                admin_roles: [],
                words: {
                    "type": "{{ $id ? __('扭蛋機編輯') : __('扭蛋機新增') }}",
                    "data_warning": '',
                },
            },
            methods: {
                get() {
                    let apiUrl = "{{ config('services.API_URL') . '/gacha/' }}" + this.id;
                    ...
                    $.ajax({
                        ...
                        dataType: 'json',
                        success: (data) => {
                            this.editData = Object.assign({}, this.editData, data.data);

                            if (!this.editData.start_time && !this.editData.end_time) {
                                this.editData.is_permanent = 1;
                            } else {
                                this.editData.is_permanent = 0;
                            }
                ...
                checkSave() {
                    ...
                    if (vc.id >= 1) {
                        method = "PUT";
                        url = "{{ config('services.API_URL') . '/gacha' }}/" + this.id;
                    }
