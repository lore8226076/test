...
class ImportSurgameItems extends Command
{
    ...
    public function handle()
    {
        ...
        if ($env == 'local') {
            $this->info('Local 環境，使用 Dev 資料');
            $baseUrl = 'https://r2dev.wow-dragon.com';
        } elseif ($env == 'qa') {
            $this->info('QA 環境，使用 QA 資料');
            $baseUrl = 'https://r2qa.wow-dragon.com';
        } else {
            $this->info('Production 環境，使用 Production 資料');
            $baseUrl = 'https://r2.wow-dragon.com';
        }

        $this->info('開始匯入surgame資料');
        $this->migrateCards($baseUrl);
        $this->migrateHeros($baseUrl);
        $this->migrateLevelUps($baseUrl);
        $this->migrateLevels($baseUrl);
        $this->migrateMobs($baseUrl);
        $this->migratePassiveRewards($baseUrl);
        $this->migrateRankFuncs($baseUrl);
        $this->migrateRankUps($baseUrl);
        $this->migrateSkills($baseUrl);
        $this->migrateStages($baseUrl);
        $this->migratePlayerLvUp($baseUrl);
        $this->migrateEquipment($baseUrl);
        $this->migrateEqEnhance($baseUrl);
        $this->migrateEqRefine($baseUrl);
        $this->migrateEqQuility($baseUrl);
        $this->migrateEqMaster($baseUrl);
        $this->migrateTalent($baseUrl);
        $this->migrateTalentDraw($baseUrl);
        $this->migrateSurgameQuest($baseUrl);
        $this->migrateGrade($baseUrl);
        $this->migrateItemPackage($baseUrl);
        $this->migrateStarRewards($baseUrl);
        $this->migrateJourneys($baseUrl);
        $this->migrateTreasure($baseUrl);
        $this->migrateDailyResource($baseUrl);
    ...
    private function migrateCards($baseUrl)
    {
        $this->info('開始匯入 Surgame Cards 資料');
        $url = $baseUrl.'/gddatabase/105_surgame_cards.txt';
    ...
    private function migrateHeros($baseUrl)
    {
        $this->info('開始匯入 Surgame Heros 資料');
        $url = $baseUrl.'/gddatabase/102_surgame_heroes.txt';
        $datas = $this->convertData($url, 'txt');
        ...
        $columnMappingData = [
            ...
            'element' => ['element', 0],
            'replace_item_id' => ['ReplaceItem', null],
        ...
        // 更新unique_id 欄位 改為 item_id
        $this->info('開始更新 Surgame Heroes unique_id 欄位');
        $heros = GddbSurgameHeroes::all();
        foreach ($heros as $hero) {
            if (empty($hero->character_id)) {
                continue;
            }
            $item = GddbItems::where(['category' => 'Hero', 'manager_id' => $hero->character_id])->first();
            if ($item) {
                $hero->unique_id = $item->item_id;
                $hero->save();
            }
        }
    ...
    private function migrateLevelUps($baseUrl)
    {
        $this->info('開始匯入 Surgame LevelUps 資料');
        $url = $baseUrl.'/gddatabase/107_surgame_levelUp.txt';
        $datas = $this->convertData($url, 'txt');
    ...
    private function migrateLevels($baseUrl)
    {
        $this->info('開始匯入 Surgame Levels 資料');
        $url = $baseUrl.'/gddatabase/106_surgame_level.txt';
        $datas = $this->convertData($url, 'txt');
    ...
    private function migrateMobs($baseUrl)
    {
        $this->info('開始匯入 Surgame Mobs 資料');
        $url = $baseUrl.'/gddatabase/103_surgame_mobs.txt';
        $datas = $this->convertData($url, 'txt');
    ...
    private function migratePassiveRewards($baseUrl)
    {
        $this->info('開始匯入 Surgame PassiveRewards 資料');
        $url = $baseUrl.'/gddatabase/115_surgame_passiveRewards.txt';
        $datas = $this->convertData($url, 'txt');
    ...
    private function migrateRankFuncs($baseUrl)
    {
        $this->info('開始匯入 Surgame RankFuncs 資料');
        $url = $baseUrl.'/gddatabase/108_surgame_rankFunc.txt';
        $datas = $this->convertData($url, 'txt');
    ...
    private function migrateRankUps($baseUrl)
    {
        $this->info('開始匯入 Surgame RankUps 資料');
        $url = $baseUrl.'/gddatabase/109_surgame_rankUp.txt';
        $datas = $this->convertData($url, 'txt');
    ...
    private function migrateSkills($baseUrl)
    {
        $this->info('開始匯入 Surgame Skills 資料');
        $url = $baseUrl.'/gddatabase/101_surgame_skills.txt';
        $datas = $this->convertData($url, 'txt');
    ...
    private function migrateStages($baseUrl)
    {
        $this->info('開始匯入 Surgame Stages 資料');
        $url = $baseUrl.'/gddatabase/104_surgame_stages.txt';
        $datas = $this->convertData($url, 'txt');
    ...
    private function migratePlayerLvUp($baseUrl)
    {
        $this->info('開始匯入 Surgame PlayerLvUp 資料');
        $url = $baseUrl.'/gddatabase/100_surgame_playerLvUp.txt';
        $datas = $this->convertData($url, 'txt');
    ...
    private function migrateEquipment($baseUrl)
    {
        $this->info('開始匯入 Surgame Equipment 資料');
        $url = $baseUrl.'/gddatabase/110_surgame_equipment.txt';
        $datas = $this->convertData($url, 'txt');
    ...
    private function migrateEqEnhance($baseUrl)
    {
        $this->info('開始匯入 Surgame EqEnhance 資料');
        $url = $baseUrl.'/gddatabase/111_surgame_eq_enhance.txt';
        $datas = $this->convertData($url, 'txt');
    ...
    private function migrateEqRefine($baseUrl)
    {
        $this->info('開始匯入 Surgame EqRefine 資料');
        $url = $baseUrl.'/gddatabase/112_surgame_eq_refine.txt';
        $datas = $this->convertData($url, 'txt');
    ...
    private function migrateEqQuility($baseUrl)
    {
        $this->info('開始匯入 Surgame EqQuility 資料');
        $url = $baseUrl.'/gddatabase/113_surgame_eq_quility.txt';
        $datas = $this->convertData($url, 'txt');
    ...
    private function migrateEqMaster($baseUrl)
    {
        $this->info('開始匯入 Surgame EqMaster 資料');
        $url = $baseUrl.'/gddatabase/114_surgame_eq_master.txt';
        $datas = $this->convertData($url, 'txt');
    ...
    private function migrateTalent($baseUrl)
    {
        $this->info('開始匯入 Surgame Talent 資料');
        $url = $baseUrl.'/gddatabase/116_surgame_talent.txt';
        $datas = $this->convertData($url, 'txt');
    ...
    private function migrateTalentDraw($baseUrl)
    {
        $this->info('開始匯入 Surgame TalentDraw 資料');
        $url = $baseUrl.'/gddatabase/117_surgame_talentDraw.txt';
        $datas = $this->convertData($url, 'txt');
    ...
    private function migrateItemPackage($baseUrl)
    {
        $this->info('開始匯入 Surgame Item Package 資料');
        $url = $baseUrl.'/gddatabase/124_surgame_itemPackage.txt';
        $datas = $this->convertData($url, 'txt');
    ...
    private function migrateGrade($baseUrl)
    {
        $this->info('開始匯入 Surgame Grade 資料');
        $url = $baseUrl.'/gddatabase/119_surgame_grade.txt';
        $datas = $this->convertData($url, 'txt');
    ...
    public function migrateSurgameQuest($baseUrl)
    {
        $this->info('開始匯入 Surgame Quests 資料');
        $url = $baseUrl.'/gddatabase/120_surgame_quest.txt';
        $datas = $this->convertData($url, 'txt');
        Tasks::truncate();
        $this->info('清空當前資料');
    ...
    // 星級挑戰
    public function migrateStarRewards($baseUrl)
    {
        $this->info('開始匯入 Surgame StarRewards 資料');
        $url = $baseUrl.'/gddatabase/123_surgame_starsRewards.txt';
        $datas = $this->convertData($url, 'txt');
    ...
    // 冒險之旅
    public function migrateJourneys($baseUrl)
    {
        $this->info('開始匯入 Surgame Journeys 資料');
        $url = $baseUrl.'/gddatabase/122_surgame_journey.txt';
        $datas = $this->convertData($url, 'txt');
    ...
    public function migrateTreasure($baseUrl)
    {
        $this->info('開始匯入 Surgame Treasure 資料');
        $url = $baseUrl.'/gddatabase/118_surgame_treasure.txt';
        $datas = $this->convertData($url, 'txt');
    ...
    public function migrateDailyResource($baseUrl)
    {
        $this->info('開始匯入 Surgame DailyResource 資料');
        $url = $baseUrl.'/gddatabase/126_surgame_Daily_Resource.txt';
        $datas = $this->convertData($url, 'txt');

        GddbSurgameDailyResource::truncate();
        $this->info('清空當前資料');
        ...
        $columnMappingData = [
            ...
            'stage_lv' => ['StageLv', 0],
            'first_reward' => ['FirstReward', 0],
            'reward' => ['Reward', 0],
        ];

        $res = $this->storeInputData(GddbSurgameDailyResource::class, $columnMappingData, $datas);
        if (! $res['success']) {
            $this->info('DailyResource 匯入失敗');

            return 0;
        }
        $this->info('DailyResource 匯入完成');
    }
                            ...
                            if (is_string($val)) {
                                ...
                                if ($trim === '' || strtoupper($trim) === 'NULL') {
                                    ...
                                    if (json_last_error() === JSON_ERROR_NONE) {
                                        ...
                                        // 嘗試解析 "[500,1],[510,1]" 這種格式（多個陣列用逗號連接）
                                        $wrappedJson = '['.$json.']';
                                        $decoded = json_decode($wrappedJson, true);
