...
{
    // 抽取天賦
    public function executeDraw($user, $sessionId, $talents = [])
    {
        try {
            DB::beginTransaction();

            // 取得當前獎池資訊（用來獲取 level_at_bind）
            ...
            if (! $talentPool) {
                ...
                return [
                    'success' => 0,
                    'error_code' => 'TALENT:0009', // 獎池不存在或已完成
                ];
            }

            // 取得花費資訊
            $costInfo = $this->getCurrentPoolCost($talentPool->level_at_bind);
            if ($costInfo === null) {
                DB::rollBack();

                return [
                    'success' => 0,
                    'error_code' => 'TALENT:0010', // 無法取得花費資訊
                ];
            }

            // 檢查資源是否足夠
            $itemId = $costInfo['cost']; // 花費的道具 ID
            $amount = $costInfo['amount']; // 花費的數量

            $checkResult = (new UserItemService)->checkResource($user->id, $itemId, $amount);
            if ($checkResult['success'] === 0) {
                DB::rollBack();

                return [
                    'success' => 0,
                    'error_code' => $checkResult['error_code'], // 資源不足或其他錯誤
                ];
            }

            // 扣除花費
            $removeResult = UserItemService::removeItem(
                UserItemLogs::TYPE_TALENT_DRAW,
                $user->id,
                $user->uid,
                $itemId,
                $amount,
            ...
            if ($removeResult['success'] === 0) {
                DB::rollBack();
                Log::error('扣除天賦抽取花費失敗', [
                    'uid' => $uid,
                    'item_id' => $itemId,
                    'amount' => $amount,
                    'error_code' => $removeResult['error_code'],
                ]);

                return [
                    'success' => 0,
                    'error_code' => 'TALENT:0011', // 扣除花費失敗
                ];
            }

            // 抽獎
            $itemCode = $this->drawTalent($user->uid, $talents);
            if ($itemCode === null) {
                DB::rollBack();
