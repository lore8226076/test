...
class TaskService
{
    ...
    {
        $userTask = UserTasks::where('uid', $uid)->where('id', $userTaskId)->latest()->first();
    ...
    public function calculateStat(string $uid, string $column, $value = null): ?int
    {
        ...
        switch ($column) {
            ...
            case 'weekly_build':
                ...
                return $value;
            case 'quick_patrol':
                return $this->patrolQuick($user->uid);
                break;
            case 'obtain_hero':
                return $this->gachaTypeCount($uid, 1);
            case 'obtain_treasure':
                return $this->gachaTypeCount($uid, 2);
            case 'draw_skin':
                return $this->gachaTypeCount($uid, 3);
            case 'play_money_maze':
                return $this->resourceStageCount($user, 'play_money_maze');
            case 'play_exp_maze':
                return $this->resourceStageCount($user, 'play_exp_maze');
            case 'play_gift_maze':
                return $this->resourceStageCount($user, 'play_gift_maze');
            case 'spend_diamond':
                return $this->diamondSpentToday($user->id);
            case 'buy_stamina':
            case 'edit_home':
            case 'main_journey':
            case 'join_party':
            case 'open_party':
                return $this->returnOne($uid);
    ...
    public function formatCompletedTasks($completedTasks)
    {
        ...
        // 軍階
        $finishedGradeTask = $completedTasks->filter(function ($task) {
            return $task->task?->type === 'grade';
        });
        ...
        return [
            ...
            'grade' => [
                'hasFinishedTasks' => $finishedGradeTask->isNotEmpty(),
            ],
        ];
    }
    ...
    public function formatTaskRewards($rewards)
    {
        ...
        // 1. 若 reward 已經是陣列
        if (is_array($rewards)) {
            // 檢查是否為二維陣列 (ex: [[500,1],[510,1],[190,30]])
        ...
        if (is_string($rewards)) {
            // 先嘗試 JSON 解碼 (ex: "[[500,1],[510,1],[190,30]]")
            ...
            // JSON 解碼失敗，使用正則表達式處理舊格式 (ex: "[100,1],[101,2]")
    ...
    /**
     * 快速巡邏次數
     * keywords: patrol_quick
     */
    public function patrolQuick($uid)
    {
        // patrol_max - patrol_count
        return UserStatus::where('uid', $uid)
            ->value(DB::raw('patrol_max - patrol_count'));
    }

    /**
     * 取得扭蛋各類型扭蛋任務當日抽取次數
     */
    public function gachaTypeCount(string $uid, string $gachaType)
    {
        $todayStart = now()->startOfDay();
        $todayEnd = now()->endOfDay();

        return UserGachaOrders::where('uid', $uid)
            ->where('type', $gachaType)
            ->whereBetween('created_at', [$todayStart, $todayEnd])
            ->exists() ? 1 : 0;
    }
     ...
     * 掃蕩或通關資源關卡
     */
    public function resourceStageCount(Users $user, $column = null)
    {
        if ($column == 'play_money_maze') {
            $passType = 'DailyMoney';
            $ItemLogType = 34;
        } elseif ($column == 'play_exp_maze') {
            $passType = 'DailyExp';
            $ItemLogType = 35;
        } elseif ($column == 'play_gift_maze') {
            $passType = 'MusesGift';
            $ItemLogType = 36;
        }

        // 1. 通關資源關卡
        $r1 = UserSurgameResourceStageRecord::where('type', $passType)
            ->where('uid', $user->uid)
            ->whereBetween('created_at', [now()->startOfDay(), now()->endOfDay()])
            ->exists() ? 1 : 0;

        // 2. 掃蕩資源關卡
        $r2 = UserItemLogs::where('user_id', $user->id)
            ->where('type', $ItemLogType ?? -1)
            ->whereBetween('created_at', [now()->startOfDay(), now()->endOfDay()])
            ->exists() ? 1 : 0;
     ...
     * 今日是否有消費鑽石
     */
    public function diamondSpentToday(string $user_id)
    {

        $todayStart = now()->startOfDay();
        $todayEnd = now()->endOfDay();
        $spent = UserItemLogs::where('user_id', $user_id)
            ->where('item_id', 100) // 鑽石ID
            ->where('qty', '<', 0) // 消費為負數
            ->whereBetween('created_at', [$todayStart, $todayEnd])
            ->exists() ? 1 : 0;
            return $spent;
    ...
    /**
     * 回傳1 的任務
     * keywords: main_journey, obtain_hero, draw_skin, obtain_treasure, play_money_maze, play_exp_maze, play_gift_maze, buy_stamina
     */
    public function returnOne(string $uid)
    {
        return 1;
    }
}
