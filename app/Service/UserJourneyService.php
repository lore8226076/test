...
class UserJourneyService
{
    ...
    public function updateJourneyProgress(int $uid, int $chapterId, int $wave): array
    {
        ...
        return DB::transaction(function () use ($uid, $journey, $wave) {
            ...
            $currentJourneyId = (int) $record->current_journey_id;
            $currentWave = (int) $record->current_wave;
            $newJourneyId = (int) $journey->unique_id;
            $newWave = max(0, $wave);
            ...
            if ($newJourneyId > $currentJourneyId) {
                // 新章節 ID 更高，直接更新
                $shouldUpdate = true;
            } elseif ($newJourneyId == $currentJourneyId) {
                // 相同章節，檢查波次是否更高
                if ($newWave > $currentWave) {
                    $shouldUpdate = true;
                }
            }
            // 如果新章節 ID 更低，不更新（但返回傳入的進度）

            if ($shouldUpdate) {
                $record->current_journey_id = $newJourneyId;
                $record->current_wave = $newWave;
                $record->save();
            }
        ...
        return DB::transaction(function () use ($user, $rewards) {
            ...
            foreach ($rewards as $reward) {
                ...
                $result = UserItemService::addItem(
                    UserItemLogs::TYPE_MAIN_CHAPTER,
                    $user->id,
                    $user->uid,
                    $itemId,
                    $amount,
