...
class StaminaService
{
    ...
    const QUICK_PATROL_STAMINA_COST = 15; // 快速巡邏體力消耗

    /**
     * 取得用戶實際體力上限（基礎上限 + 月卡加成）
     *
     * @param  int  $uid  用戶 UID
     * @return int
     */
    public static function getMaxStamina($uid): int
    {
        $status = UserStatus::where('uid', $uid)->first();
        $baseMax = $status ? $status->stamina_max : self::STAMINA_MAX;

        // 取得月卡額外體力上限加成
        $extraStamina = MonthCardService::getExtraStaminaMax($uid);

        return $baseMax + $extraStamina;
    }
    ...
    public static function getStamina($uid, $fullData = false)
    {
        ...
        // 使用包含月卡加成的體力上限
        $maxStamina = self::getMaxStamina($uid);
        ...
        $data = [
            ...
            'base_max_stamina' => $status->stamina_max,
    ...
    public static function syncStamina($uid)
    {
        ...
        // maxStamina 已在上方透過 getMaxStamina 取得
