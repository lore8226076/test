...
class AuthController extends Controller
{
    ...
    public function firebase_login(Request $request)
    {
        ...
        // 新手教學設定已完成
        if (empty($user->teaching_level)) {
            $user->teaching_level = 1;
            $user->save();
        }
        ...
        // 初始化UserStatus
        $this->initializeUserStatus($user);

        // 初始化活動信件
        if (! $this->checkLoginEventMail($user)) {
            $this->createLoginMail($user);
        }

        // 初始化貨幣
        UserItemService::initCurrency($user);
    ...
    public function login(Request $request)
    {
        ...
        $initResult = $this->initializeUserSurgameData($user);
        // ============ 任務系統 ============
        $userSurgameInfo = UserSurGameInfo::where('uid', $user->uid)->first();
        $formattedTaskResult = $this->handleUserTasks($user, $userSurgameInfo);
        // ============ 任務系統 ============
    ...
    public function userInfo($uid)
    {
        ...
        // 檢查是否有精煉/強化資料
        if (! $this->checkUserDeploySlotEquip($currentUser->uid)) {
            $this->initSlotEquip($currentUser->uid);
        }

        // 強制設定主角在0號位置
        $svc = app(DeploySlotService::class);
        $checkMain = $svc->checkAndSetMainCharacter($currentUser->uid);
        if (! $checkMain) {
            $svc->forceSetMainCharacter($currentUser->uid);
        }
    ...
    public function userSurgameInfo($uid)
    {
        ...
        // 取得玩家掃蕩&巡邏資料
        $userStatus = UserStatus::where('uid', $uid)->first(['patrol_count']);
        $surgameInfo->patrol_count = $userStatus?->patrol_count ?? 0;
        $surgameInfo = $surgameInfo->makeHidden(['grade_level']);
        $surgameInfo->patrol_deduction_stamina = StaminaService::QUICK_PATROL_STAMINA_COST;

        // 取得月卡資料
        $monthCardInfo = MonthCardService::getUserMonthCardBonuses($uid);

        $surgameInfo->enable_3x_speed = $monthCardInfo['enable_3x_speed'] ?? 0; // 三倍速
        $surgameInfo->unlimited_quick_patrol = $monthCardInfo['unlimited_quick_patrol'] ?? 0; // 無限掃蕩
        $surgameInfo->enable_patrol_reward = $monthCardInfo['enable_patrol_reward'] ?? 0; // 巡邏獎勵加成
        $surgameInfo->patrol_reward_percent = $monthCardInfo['patrol_reward_percent'] ?? 0; // 巡邏獎勵百分比
        $surgameInfo->stage_reward_percent = $monthCardInfo['stage_reward_percent'] ?? 0; // 關卡獎勵百分比
    ...
    public function initializeUserDeploySlots($uid)
    {
        ...
        // 若已有2個以上角色就直接結束
        if ($filledCount >= 3) {
            return ['success' => true, 'message' => '角色已放置'];
        }

        // Step 2: 發送初始角色（只發 2950, 3950, 1950 三個角色）
        $defaultCharacterIds = [2950, 3950, 1950];
        ...
        // Step 3: 初始化陣位角色（pos1->2950, pos2->3950, pos3->1950）
        ...
        foreach ($positionMapping as $position => $characterId) {
            $slot = CharacterDeploySlot::where('uid', $uid)->where('position', $position)->first();
            if (! $slot) {
                continue;
            }

            if (empty($slot->character_id) && isset($userCharacters[$characterId])) {
                $slot->character_id = $characterId;
                $slot->save();
            }
        }

        // Step 4: 更新角色為 has_use, slot_index
        foreach ($userCharacters as $userCharacter) {
            $deploySlot = CharacterDeploySlot::where('uid', $uid)
    ...
    // 初始化相關資料
    private function initializeUserSurgameData($user)
    {
        try {
            // 初次登入贈送邏輯
            if (empty($user->new_give)) {
            ...
            \Log::error('[initializeUserSurgameData] 初始化玩家資料失敗', [
                'uid' => $uid,
                'error' => $e->getMessage(),
            ]);
    ...
    // UserStatus沒有資料 就初始化UserStatus
    private function initializeUserStatus($user)
    {
        ...
        if (empty($userStatus)) {
            try {
                UserStatus::create([
                    'uid' => $user->uid,
                    'stamina' => StaminaService::STAMINA_MAX,
                    'stamina_max' => StaminaService::STAMINA_MAX,
                    'last_stamina_recover_time' => Carbon::now(),
                ]);
            } catch (Exception $e) {
                \Log::error('[initializeUserStatus] 初始化UserStatus失敗', [
                    'uid' => $user->uid,
                    'error' => $e->getMessage(),
                ]);
            }
     ...
     * 2. 沒有信就新增信件
     */
    private function checkLoginEventMail($user)
    {
        return UserInboxEntries::where('uid', $user->uid)
            ->where('inbox_messages_id', 20)
            ->exists();
    }

    private function createLoginMail($user)
    {
        $existingMail = InboxMessages::where('id', 20)->exists();
        if (! $existingMail) {
            \Log::error('登入活動信ID不存在', [
                'uid' => $user->uid,
                'inbox_messages_id' => 20,
            ]);
