...
class TaskController extends Controller
{
    ...
    public function claimAllRewards()
    {
        ...
        foreach ($userTasks as $userTask) {
            ...
            if ($result) {
                $taskData = Tasks::find($userTask->task_id);
                if (empty($taskData)) {
                    continue;
                }
    ...
    private function parseReward($rawReward): array
    {
        ...
        if (is_string($rawReward)) {
            ...
            // 處理 "[100,2],[103,2]" 格式（多個陣列用逗號連接，沒有外層括號）
            if (preg_match('/^\[[\d,]+\],\[/', $rawReward)) {
                $rawReward = '['.$rawReward.']';
            }
        ...
        // Step 2: 若還是解析不到或不為陣列，直接回傳空陣列
        if (! is_array($reward)) {
            return [];
        }

        // Step 3: 統一轉換成 [['item_id'=>x, 'amount'=>y]]
        $formatted = [];

        // 單筆獎勵 [id, amount]
        if (count($reward) === 2 && is_numeric($reward[0] ?? null) && is_numeric($reward[1] ?? null)) {
            return [[
